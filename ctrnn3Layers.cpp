...
void Ctrnn3Layers::compute_genotype_length ( void ){
  ...
  periphery_nodes=num_input-num_output;  
    //weights input-hidden and outputs
 for(int i = 0; i < num_input; i++){
        if(i<periphery_nodes){
            for(int o= 0; o <motor_nodes; o++)
               genotype_length++;
        }
    ...
    //weights hidden-hidden and for memory 
    for(int h = 0; h < num_hidden; h++)
...
void Ctrnn3Layers::allocate(int numInput, int numHidden, int numOutput)
{
        ...
        for( int i=0; i<num_hidden; i++) {
          ...
          hiddenLayer[i].weightsSelf  = new double[num_hidden];  //for memory 
        }
        for( int i=0; i<num_output; i++){
          outputLayer[i].weightsSelf  = NULL;
          outputLayer[i].weightsOut   = NULL;
        }
        sensorsGain           = new double[num_input];  //for memory 
...
void Ctrnn3Layers::update_hidden_layer( ){
  for(int i=0; i < num_hidden; i++) {
    hiddenLayer[i].s = -hiddenLayer[i].state;
          ...
          int to_afferent_node = num_input;
    update_StimesW  ( i, from_afferent_node, to_afferent_node, inputLayer, &(hiddenLayer[i].s) );
...
void Ctrnn3Layers::update_output_layer ( ){
  ...
  for(int i=0; i < num_output; i++) {
    outputLayer[i].s = -outputLayer[i].state;
           ...
           int to_afferent_node_input = periphery_nodes;// from 0 to 236 etc.
    ...
    update_StimesW  ( i, from_afferent_node_input,to_afferent_node_input , inputLayer, &(outputLayer[i].s) );
     }
     update_StimesW  ( i, from_afferent_node_hidden,to_afferent_node_hidden , hiddenLayer, &(outputLayer[i].s) );
  }
  for(int i=0; i < num_output; i++)
...
void Ctrnn3Layers::update_StimesW ( int i, int from_afferent_node,int to_afferent_node, vector<Node> &layer, double *s ){
  for(  int j = from_afferent_node; j <to_afferent_node; j++ ) {
