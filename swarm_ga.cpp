...
Swarm_ga::Swarm_ga ( Experiment* experiment ) : GA( experiment ){
  ...
  for(int c=0; c < num_tot_solutions; c++){
    ...
    queens_chromo[c].init_allele_values ( m_experiment->getNetworksNbGenes() /*num_alleles_*/, num_bases_per_allele );
    tmp_queens_chromo[c].set_diploid ( true );
    
    drones_chromo[c].set_diploid ( false );
    drones_chromo[c].init_allele_values ( m_experiment->getNetworksNbGenes() /*num_alleles_*/, num_bases_per_allele );
...
void Swarm_ga::dumpToFile( const char* fileDir, const char* fileName ){
  dump_queens_genome( fileDir, fileName, m_current_generation );
  dump_drones_genome( fileDir, fileName, m_current_generation );
}
...
void Swarm_ga::runOneGeneration ( void ){

}

/* -------------------------------------------------------------------------------------- */

void Swarm_ga::computeFitnessForTheRange (const unsigned int begin, const unsigned int end){
